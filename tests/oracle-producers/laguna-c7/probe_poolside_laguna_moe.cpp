...
static constexpr int kMaxTokens = 22;
static constexpr int kLayers = 48;
static constexpr int kVocab = 100352;

static constexpr std::array<int32_t, kMaxTokens> kExpectedTokens = {
...
static void decode_short_prompt(
    ...
    batch.n_tokens = state.token_count;
    for (int index = 0; index < state.token_count; index++) {
        batch.token[index] = tokens[static_cast<size_t>(index)];
        batch.pos[index] = index;
        batch.n_seq_id[index] = 1;
        batch.seq_id[index][0] = 0;
        batch.logits[index] = index == state.token_count - 1;
    }
...
int main(int argc, char **argv) {
    ...
    try {
...

        {
            ...
            BatchGuard batch(options.token_count);
        ...
        if (options.detail_layer == 0) {
            std::printf("embedding=embd.f32\nlayer0_checkpoints=14\nlayers=48\n"
                        "tokens=%d\nlogits=logits.f32\nout=%s\n",
                        options.token_count, options.out.c_str());
        } else {
            std::printf("embedding=embd.f32\ndetail_layer=%d\n"
                        "detail_checkpoints=14\nlayers=48\ntokens=%d\n"
                        "logits=logits.f32\nout=%s\n",
