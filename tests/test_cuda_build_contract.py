...
class CudaBuildContractTest(unittest.TestCase):
    ...
    def test_laguna_decode_vec_has_bounded_topology_oracle_shape_and_alignment_fallback(
        ...
        self.assertIn("ds4_gpu_tensor_read(", unaligned_case)
        self.assertIn("laguna_parity_spans_within_limits(", unaligned_case)
        self.assertIn("if (frozen_name &&", LAGUNA_KERNEL_TEST)
        self.assertNotIn("c->key_count == 1u", LAGUNA_KERNEL_TEST)
