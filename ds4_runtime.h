...
/* Publish the exact client-visible subset after the protocol projection has
 * inspected the complete generated byte stream.  Unlike the incremental
 * helper above, this one-shot operation accepts a historical decode window
 * even when page-advice observations occurred between those decodes and the
 * final projection. */
